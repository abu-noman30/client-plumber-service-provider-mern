		...
		<>
			...
			<div className='service-card mb-4 rounded-lg shadow-xl shadow-gray-700' data-aos='zoom-in'>
				<div className='group h-full border-gray-500 border-opacity-60 border-2 rounded-lg bg-white overflow-hidden shadow-lg'>
					...
					<PhotoProvider>
						<PhotoView src={img}>
							<img className='lg:h-48 md:h-36 w-full object-cover object-center transition duration-500 ease-in-out transform group-hover:scale-105' src={img} alt='blog' />
						</PhotoView>
					</PhotoProvider>
					...
					<div className='py-1 px-6'>
						{/* ::Card title */}
						<h1 className='mb-3 inline-block title-font text-xl font-extrabold text-black tracking-wide cursor-pointer'>{name}</h1>
						{/* ::Card excerpt */}
						<p className='line-clamp-6 mb-3 overflow-hidden leading-relaxed text-zinc-500 cursor-pointer break-all'>{`${description.slice(0, 100)}...`}</p>
					</div>
